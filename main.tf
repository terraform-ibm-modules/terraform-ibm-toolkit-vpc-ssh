locals {
  ...
  public_key         = local.keys_provided ? var.public_key : trimspace(tls_private_key.generated_key[0].public_key_openssh)
  private_key        = local.keys_provided ? var.private_key : trimspace(tls_private_key.generated_key[0].private_key_pem)
}
