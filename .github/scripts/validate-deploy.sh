...
PROVIDED_PUBLIC_KEY=$(cat terraform.tfvars | grep "public_key=" | sed -E 's/public_key="(.*)".*/\1/g')
PROVIDED_PRIVATE_KEY=$(cat terraform.tfvars | grep "private_key=" | sed -E 's/private_key="(.*)".*/\1/g')
